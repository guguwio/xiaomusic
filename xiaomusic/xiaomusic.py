...
class XiaoMusic:
    ...
    async def get_volume(self, did="", **kwargs):
        control_did = self.device_manager.get_control_did(did)
        return await self.device_manager.devices[control_did].get_volume()

    # 获取完整播放状态
    async def get_player_status(self, did="", **kwargs):
        control_did = self.device_manager.get_control_did(did)
        return await self.device_manager.devices[control_did].get_player_status()
    ...
    async def set_volume(self, did="", arg1=0, **kwargs):
        ...
        control_did = self.device_manager.get_control_did(did)
        volume = int(arg1)
        return await self.device_manager.devices[control_did].set_volume(volume)

    async def pause_playback(self, did="", **kwargs):
        control_did = self.device_manager.get_control_did(did)
        control_device_id = self.device_manager.devices[control_did].device_id
        return await self.device_manager.devices[did].pause_playback(control_device_id)

    async def resume_playback(self, did="", **kwargs):
        return await self.device_manager.devices[did].resume_playback()
