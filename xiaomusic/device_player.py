...
class XiaoMusicDevice:
    ...
    async def group_player_play(self, url, name=""):
        """同一组设备播放"""
        device_id_list = self.xiaomusic.device_manager.get_group_play_device_id_list(
            self.group_name, self.device_id
        )
        tasks = [
    ...
    async def pause_playback(self, device_id=None):
        """暂停设备播放，不清空播放状态。"""
        device_id = device_id or self.device_id
        self.log.info(f"pause_playback. did:{self.did} device_id:{device_id}")
        try:
            ret = await self.auth_manager.mina_service.player_pause(device_id)
            self.is_playing = False
            await self.cancel_next_timer()
            self.log.info(f"pause_playback ret:{ret}")
            return ret
        except Exception as e:
            self.log.exception(f"Execption {e}")
        return None
    ...
    async def resume_playback(self):
        ...
        if not cur_music:
            return await self._play_next()
        return await self._playmusic(cur_music)
